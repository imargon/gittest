...
update rep.d_user_change t1
   set t1.jsrq =to_date('2016-05-25','yyyy-mm-dd')
   where t1.bmdm = '138';--112

insert into rep.d_user_change values
  ('1549',
   '胡利鸿',
   '112',
   '2',
...
select distinct(a.bmdm),b.name,b.ind
 from D_UNIT_SUB_SUM a, rep.B_M_SYS_BRAN b
 where a.bmdm=b.code;--->106

select * from rep.d_user_change t1 where t1.zgmc ='高青洲'
 for update;  
